...
class ACTrainer:
    def __init__(self,
        ...
        self.accumulation_steps = max(self.cfg.training.actor_critic.grad_acc_steps,
                                      self.cfg.training.actor_critic.collection_steps)
    ...
    def episode_end(self, epoch=None, collection=False, do_step=True):
        ...
        if do_step:  # speed up
            ...
            self.metrics["means"] = self._create_hist(episode_output.continuous_means[mask_paddings], names=ACTION_C_NAMES)
            self.metrics["stds"] = self._create_hist(episode_output.continuous_stds[mask_paddings], names=ACTION_C_NAMES)
    ...
    def ac_loss(self, outputs: ImagineOutput, mask_paddings, epoch=None):
        ...
        #                                                         mean_line=True)
        self.metrics["value_mean"] = values[mask_paddings].mean().item()
        ...
        loss_entropy = torch.masked_select(
            ...
            mask_paddings.unsqueeze(-1)).mean() + \
            torch.masked_select(  # logit l2 regularization loss. cat(logits=[10,10,-10,-10]).entropy() is 0.69!!
                self.cfg.training.actor_critic.entropy_weight * outputs.logits_actions[..., ACTION_LOGIT_LOCK_MASK]**2,
...
def main(cfg: DictConfig):
    ...
    for f in Path("checkpoints\dataset").glob("*"):
        if int(f.stem) > cfg.training.actor_critic.batch_num_samples:
            f.unlink()
    trainer.train_dataset.load_disk_checkpoint(trainer.ckpt_dir / 'dataset')

    # shutil.copytree(
    #     Path(r"C:\Users\Michael\PycharmProjects\Brawl-Stars-AI\outputs\prioritization_and_accumulation\2024-03-27_23-33-28\checkpoints"),
    ...
    for n_episode in range(start_epoch, MAX_EPISODES):
        for _ in range(cfg.training.actor_critic.collection_steps-1):
            collect_one(do_replay=True, do_step=False, do_log=True, verbose=10)

        ac_trainer.replay_only = True
        for _ in range(cfg.training.actor_critic.grad_acc_steps-cfg.training.actor_critic.collection_steps):
            ac_trainer.reset()
            episode_metrics = ac_trainer.episode_end(do_step=False)
            wandb.log({"epoch": n_episode, **episode_metrics})
        ac_trainer.replay_only = False

        collect_one(do_replay=True, do_step=True, do_log=True, verbose=10)  # ensure 1/1 step config works
...
def main_replay(cfg: DictConfig):
    ...
    for n_step in tqdm.tqdm(range(start_epoch, MAX_EPISODES), desc="Step: ", total=MAX_EPISODES, ):
        ac_trainer.reset(prioritize=False, zero_center=True)
...
def inspect(cfg: DictConfig):
    try:
        ...
        shutil.copytree(
            Path(
                r"C:\Users\Michael\PycharmProjects\Brawl-Stars-AI\outputs\entropy_fix\2024-03-26_22-08-34\checkpoints"),
            Path("checkpoints"), dirs_exist_ok=True, ignore=shutil.ignore_patterns("checkpoint*"))
        trainer.load_checkpoint()
